...
import { v4 } from 'uuid';
import ImagesUploadWithEdit from './ImagesUploadWithEdit';
import UserAvatar from '../../../general/UserAvatar';
import { arrayLengthCount, getUserName, notEmptyLength } from '../../../../common-function';
...
const WritePostModal1 = (props) => {
    ...
    function onSubmit() {
        ...
        for (let i = 0; i < fileList.length; i++) {
            ...
            if (_.get(fileList , `[${i}].originFileObj`)) {
        ...
        Promise.all(promiseArr)
            .then((resArr) => {
                ...
                for (let i = 0; i < resArr.length; i++) {
                    if (_.get(resArr , `[${i}].originFileObj`)) {
                        uploadYes = 'yes'
                ...
                if (uploadYes === 'yes') {
                    ...
                    uploadPromiseArr.push(
                        axios.post(`${client.io.io.uri}upload-images-array`,
                            ...
                            fileList = _.map(fileList, function (v) { if (!v.originFileObj) { return v } });
    ...
    return (
        <React.Fragment>
            ...
            >
                <div className="padding-md background-white">
                            ...
                            <span className='d-inline-block' >
                                <div className="flex-justify-start flex-items-align-center margin-bottom-md">
                                    ...
                                    <span className='d-inline-block' >
                                        ...
                                        {
                                            chatType == 'carfreaks' ?
                                                <ImagesUploadWithEdit
                                                    onSubmit={(images) => {
                                                        if (arrayLengthCount(imageList) <= IMAGE_LIMIT) {
                                                            setImageList(_.concat(imageList, images).slice(-IMAGE_LIMIT))
                                                        }
                                                    }}
                                                >
                            ...
                            _.isArray(imageList) && !_.isEmpty(imageList) && chatType == 'carfreaks' ?
                                ...
                                >
                                    ...
                                    {imageList.map((item, idx) => (
                                        <span className="d-inline-block margin-sm relative-wrapper" style={{ width: 100, height: 100 }} key={'image' + idx}>
                                            <img src={_.get(item, `url`)} alt={`post-uploaded-image-${idx}`} className="img-cover fill-parent" />
                                            <div className="fill-parent absolute-center background-black-opacity-50-gradient-bottom relative-wrapper flex-justify-space-between">
                                                <span className='d-inline-block ' >
                                                    {
                                                        idx === 0 ?
                                                            <span className='white'>{idx + 1 + '/Main'}</span>
                                                            : <span className='white'>{idx + 1}</span>
                                                    }
                                                </span>
                                                <span className='d-inline-block ' >
                                                    <Tooltip title="Click to Edit">
                                                        <Icon onClick={() => { setSelectedImage(item); setImageEditMode(true); }} className="white margin-right-xs" type="edit" theme="filled" />
                                                    </Tooltip>
                                                    <Tooltip title="Click to Remove">
                                                        <Icon onClick={() => { handleImageRemove(item) }} className="white" type="close-circle" theme="filled" />
                                                    </Tooltip>
            ...
            >
                <ImageEdit
                    src={_.get(selectedImage, `url`)}
                    height={450}
                    onConfirm={(output) => {
                        setImageList(
                            _.map(imageList, (image) => {
                                if (_.get(selectedImage, `uid`) == _.get(image, `uid`)) {
